...
@section('content')
    <main class="checkoutPage">
        <div class="container">
            <form class="box orderForm">
                <div class="box_left">
                    <div class="titles">{{ $page->sub_title ?: $page->title }}</div>
                    <div class="form">
                        <div class="form_row liso">
                            <div class="fiz">
                                <input type="radio" id="id1" name="ip_account" checked value="0">
                                <label for="id1">Физическое лицо</label>
                            </div>
                            <div class="ur">
                                <input type="radio" id="id2" name="ip_account" checked value="1">
                        ...
                        <div class="form_row grid">
                            ...
                            <div class="input">
                                ...
                                <input id="org_name" type="text" placeholder="Название организации" name="data[org_name]">
                            </div>
                            <div class="input">
                                <label for="org_address">Адрес организации*</label>
                                <input id="org_address" type="text" placeholder="Адрес организации" name="data[org_address]">
                            </div>
                        </div>
                        ...
                        @foreach(\App\Enums\OrderDeliveryEnum::labels() as $type => $delivery)
                            <div class="dostavka">
                                <input type="radio" id="dos{{$counter}}" name="delivery" value="{{$type}}"
                        ...
                        <div class="items">
                            @foreach($items as $item)
                                <div class="item">
                                    ...
                                    <div class="item_right">{{ $item->price * $item->quantity }}
                                        <div class="currency"> ₸</div>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    <div class="sposob">
                        ...
                        @foreach(\App\Enums\OrderPaymentEnum::labels() as $val => $payment)
                            <div class="sposob_item">
                                <div class="radio">
                                    <input type="radio" id="sp{{$count}}" name="payment" value="{{$val}}"
                                           @if($count == 1) checked @endif>
                                    <label for="sp{{$count}}">
...
@section('scripts')
    <script>
        $(function () {
            ...
            // var successMsg = localStorage.getItem('checkoutSuccess');
            // if (successMsg && successMsg !== '') {
            //     localStorage.setItem('checkoutSuccess', '');
            //     setTimeout(() => {
            ...
            $(".orderForm").on('submit', function (e) {
                ...
                $('.loadingZakaz').addClass('active');
                $('.loadingZakaz .loading .loading_box').addClass('active');
                let form = $(this);
                $.ajax({
                    headers: {
                    ...
                    success: function(response){
                        ...
                        form[0].reset();
                        // localStorage.setItem('checkoutSuccess', response.message)
                        $('.loadingZakaz .loading .loading_box').removeClass('active');
                        $('.loadingZakaz .loading .error').removeClass('active');
                        $('.loadingZakaz .loading .succes').addClass('active');
                        // window.location.reload()
                        if(response.is_guest === false) {
                            $('.loadingZakaz .loading .succes .text_havntUser').removeClass('active');
                            $('.loadingZakaz .loading .succes .text_haveUser').addClass('active');
                            setTimeout(()=>{
                                window.location.href='/profile/history';
                            }, 5000)
                        }
                        else {
                            $('.loadingZakaz .loading .succes .text_haveUser').removeClass('active');
                            $('.loadingZakaz .loading .succes .text_havntUser').addClass('active');
                            setTimeout(()=>{
                                window.location.reload()
                            }, 3000)
                        }
                    ...
                    error: function(data) {
                        console.log(data)
                        if (data.status === 422) {
                            // toastr.error('Заполните все необходимые поля!')
                            let errors = data.responseJSON.errors;
                            for (var key in errors) {
                                $(".orderForm input[name='"+ key +"']").addClass('error');
                            }
                        }
                        ...
                        $('.loadingZakaz .loading .loading_box').removeClass('active');
                        $('.loadingZakaz .loading .success').removeClass('active');
