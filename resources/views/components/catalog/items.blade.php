                    ...
                    <a href="{{ route('product.show', ['category' => $product->category->slug, 'slug' => $product->slug]) }}">
                        <picture>
                            @if($photo = \App\Helpers\Common::getWebpByImage($img))
                                <source srcset="{{ \App\Helpers\Common::getImage($photo) }}" type="image/webp">
                                <source srcset="{{ \App\Helpers\Common::getImage($photo) }}" type="image/pjp2">
                            @endif
                            <img src="{{ \App\Helpers\Common::getImage($img) }}" alt="slider image{{$k}}" loading="lazy">
                        </picture>
                    </a>
                @endforeach
            </div>
